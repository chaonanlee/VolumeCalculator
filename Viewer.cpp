...
bool flag= false;
double avgDepth=0;
openni::DepthPixel* bgDepths=new openni::DepthPixel[640*480];
...
void SampleViewer::display()
{
		...
		m_eViewState == DISPLAY_MODE_DEPTH) && m_depthFrame.isValid())
	{
		...
		switch (messageCode){
            case 1:
                std::cout<<"depth "<<pDepthRow[250+112*width]<<std::endl;
                break;
            case 2:
                getVolume(cropOriginX,cropOriginY,cropWidth,cropHeight,width,avgDepth,m_depthStream,pDepthRow);
                break;
		}
	...
	case 'm':
	    ...
	    case 'i':
	        ...
	        if(flag){
	            ...
	            memcpy(bgDepths,m_depthFrame.getData(),640*480);
                float cropOriginWX=0.0,cropOriginWY=0.0,cropOriginWZ=0.0;
                openni::CoordinateConverter::convertDepthToWorld(m_depthStream,cropOriginX,cropOriginY,bgDepths[cropOriginX+cropOriginY*640],&cropOriginWX,&cropOriginWY,&cropOriginWZ);
                float cropOriginWX1=0.0,cropOriginWY1=0.0,cropOriginWZ1=0.0;
                openni::CoordinateConverter::convertDepthToWorld(m_depthStream,cropOriginX+cropWidth,cropOriginY,bgDepths[cropOriginX+cropWidth+cropOriginY*640],&cropOriginWX1,&cropOriginWY1,&cropOriginWZ1);
                pixelArea=((cropOriginWX1-cropOriginWX)/cropWidth)*((cropOriginWX1-cropOriginWX)/cropWidth);
                messageCode=0;
	        }
            break;
        case 'p':
	    ...
	    case 'g':
            ...
            for (int y = cropOriginY; y < cropOriginY+cropHeight; ++y) {
                for (int x = cropOriginX; x < cropOriginX+cropWidth; ++x) {
                    int actual=pDepthRow[x+y*m_depthFrame.getWidth()];
                    int bgDepth=bgDepths[x+y*m_depthFrame.getWidth()];
                    if(actual!=0&&bgDepth!=0&&(abs(actual-bgDepth)>ERROR)){
                        std::cout<<bgDepth-actual<<" ";
                    } else{
                        std::cout<<" ";
                    }
...
double SampleViewer::getVolume(int cropOriginX,int cropOriginY,int cropWidth,int cropHeight,int width,double avgDepth,const openni::VideoStream& videoStream, const openni::DepthPixel* pixel) {
    double sum=0.0;
    std::cout<<"area= "<<pixelArea*cropWidth*cropHeight<<std::endl;
    for (int y = cropOriginY; y < cropOriginY+cropHeight; ++y) {
        for (int x = cropOriginX; x < cropOriginX+cropWidth; ++x) {
            int actual=pixel[x+y*width];
            int bgDepth=bgDepths[x+y*width];
            if(actual!=0&&bgDepth!=0&&(abs(actual-bgDepth)>ERROR)){
                sum+=(bgDepth-actual)*pixelArea;
            }
